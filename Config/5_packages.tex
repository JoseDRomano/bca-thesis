...
% Comando para a diferença colorida
% Uso: \diff{valor_atual}{valor_antigo}
\newcommand{\diff}[2]{%
    \pgfmathsetmacro{\res}{#1-#2}% Calcula a diferença
    \ifdim \res pt > 0pt
        {\scriptsize\color{posgreen} (+\pgfmathprintnumber[fixed, precision=2]{\res})}%
    \else
        \ifdim \res pt < 0pt
            {\scriptsize\color{negred} (\pgfmathprintnumber[fixed, precision=2]{\res})}%
        \else
            {\scriptsize\color{gray} (0.00)}%
        \fi
    \fi
}

\definecolor{validated}{RGB}{198,239,206}   % green fill  -- doubly validated
\definecolor{validatedtext}{RGB}{0,97,0}     % green text  -- doubly validated
\definecolor{clinicalfill}{RGB}{255,235,156} % amber fill  -- clinical feature
\definecolor{clinicaltext}{RGB}{156,87,0}    % amber text  -- clinical feature
...
\usepackage{multirow}
\usepackage{pgf-pie} % Requires pgf-pie package
