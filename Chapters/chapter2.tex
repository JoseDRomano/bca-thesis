...
the field of \gls{mirna} research and its applications in \gls{bc} as a
...
central dogma of molecular biology, the role of miRNA in gene expression
regulation, and the characteristics of breast cancer and its subtypes. The second
section will review the related work in the field of miRNA research,
focusing on the use of miRNA as biomarkers and subtype classifiers in
...
that does not code for proteins. These non-coding \gls{rna} molecules
play crucial regulatory roles in controlling gene expression. One of the most
studied groups within this class are miRNA, which appear to be central
elements in the fine-tuning of the genetic regulation process.

\subsection{MicroRNAs - The Regulators of Gene Expression}
\label{sec:microRNA}
miRNA are small non-coding \gls{rna} molecules, approximately $20$
...
\begin{figure}[h]
  ...
  \caption{This figure shows the process of gene expression: DNA is transcribed
    into mRNA, which is then translated into protein by the ribosome. miRNA
    are shown as regulators acting on the mRNA before translation.}
  \label{fig:mirna_mechanism}
\end{figure}

In simple terms, \textbf{miRNA function as molecular switches that bind
...
A study made by \textcite{Gulyaeva2016Regulatory} demonstrates the high
...
than $60\%$ of human coding genes are targeted for regulation by miRNA.

Given this broad regulatory capacity, miRNA play a central role in
multiple cellular processes such as proliferation, differentiation, apoptosis,
and stress response. Consequently, changes in miRNA expression profiles
are associated with several diseases, including cancer, neurodegenerative and
cardiovascular diseases. In an oncological context, miRNA can act as
...
molecular mechanisms, miRNA have been extensively investigated as
...
acquisition of invasive and metastatic capacity - properties that
...
and one of the most dynamic areas of biomedical research \cite{bc_molecular_Perou2000}. Its relevance stems
not only from its high incidence and mortality rates \cite{bcaData2024_bray, bca_data_Arnold2022Current}, but above all from its
...
same clinical subtype may respond differently to the same treatments \cite{bc_subtypes_Haque2012}.
...
molecular targets, substantial 
limitations remain, particularly related to
...
nucleic acid-based therapies stand out \cite{_Mendes2022Nanodelivery}, which aim to directly modulate the gene
expression involved in tumour progression, constituting a paradigm shift in the
treatment of breast cancer \cite{Mendes2024Nanoparticles}.

\subsection{Breast Cancer \& its Subtypes}
\label{sec:breast_cancer}
...
as seen in \textcite{updatedbca_Hong2022Breast}. It is estimated that \textbf{one in eight women}
...
\begin{itemize}
  ...
  \item Environmental and behavioral factors, such as obesity, physical inactivity,
        ...
        \cite{clinical_implication_bca_Adamo2015}.
\end{itemize}
...
diagnosed in its early stages, breast cancer has survival rates of over $90\%$.
...
\textcite{_Mendes2022Nanodelivery}.
...
of breast tumors, several studies have established that breast cancer is not a
single disease but rather a collection of biologically distinct entities that
arise from a common anatomical site
\cite{clinical_implication_bca_Adamo2015,clinical_implication_bca_Adamo2015}. This
...
\cite{bc_molecular_Perou2000,clinical_implication_bca_Adamo2015}. These are typically
...
gene expression patterns. The relationship between breast cancer subtypes and these
...
\begin{table}
  \centering
  \small
  \caption{Summary of intrinsic breast cancer subtypes and typical characteristics of each one.\newline\textit{References:} \cite{clinical_implication_bca_Adamo2015}, \cite{diff_bca_usa_Howlader2018}, \cite{clinical_implication_bca_Adamo2015}, \cite{updatedbca_Hong2022Breast}.}
  \label{tab:bc_subtypes_summary}
  \begin{tabular}{llll}
    \toprule
    \textbf{Subtype}  & \textbf{Receptors / HER2} & \textbf{Prolif.} & \textbf{Prognosis} \\
    \midrule
    Luminal A         & ER+/PR+, HER2−            & Low              & Favorable          \\
    Luminal B         & ER+, PR↓, HER2±           & High             & Intermediate       \\
    HER2-enriched     & HER2+, ER−, PR−           & High             & Improved           \\
    TNBC              & ER−, PR−, HER2−           & High             & Poor               \\
    \bottomrule
  \end{tabular}
...
\cite{bc_molecular_Perou2000, clinical_implication_bca_Adamo2015}.

One of the most striking examples of these limitations is the triple-negative 
subtype, characterized by the absence of estrogen, progesterone, and HER2 
receptors, which eliminates the possibility of hormonal or targeted therapies 
and leaves chemotherapy as the main treatment option \cite{Yin2020Triple-negative}. 
Despite initial responses, relapses are frequent and trials with agents such as 
bevacizumab have shown only marginal benefits, resulting in the lowest 
subtype-specific survival rate among all breast cancer subtypes 
\cite{diff_bca_usa_Howlader2018, Eroles2012Molecular}.
...
approaches, these are the points that also form the basis for research into computational
...
developed to date to explore the potential of miRNA as biomarkers in the
context of oncology, covering both breast cancer and other malignant neoplasms. The
...
high-dimensional data, including the expression of miRNA in cancer
...
breast cancer subtypes based on miRNA expression profiles, complementing them
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{Chapters/Figures/NN.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1.0\textwidth]{Chapters/Figures/CNN_derma.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{Chapters/Figures/taxonomy.png}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.50\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapters/Figures/conf-matrix-docs.png}
  ...
  \begin{subfigure}[b]{0.50\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Chapters/Figures/AUC_derma_2.png}
...
\begin{figure} [h]
  \centering
  \includegraphics[width=0.5\textwidth]{Chapters/Figures/t_sne_derma.png}
...
also extends to the field of molecular data, as we will see in the next sections.

% This capability is not limited to imaging data: it
...
% learning algorithms to the task of classifying breast cancer subtypes (in this case,
...
% demonstrating the potential of \gls{ai} in classifying breast cancer subtypes based
...
% (TCGA) \cite{TCGA_BRCA}, a public database containing thousands of tumor samples characterized
...
% \begin{figure} [h]
%   \centering
%   \includegraphics[width=0.7\textwidth]{Chapters/Figures/overfit.png}
...
%         \begin{figure} [H]
%           \centering
%           \includegraphics[width=0.45\linewidth]{Chapters/Figures/knn-example.jpg}
...
%   \item \textbf{Decision Tree (\textit{DT})}: a model that makes decisions through a hierarchical tree-shaped structure, where each internal node represents a condition on a variable, and each branch represents a possible outcome of that condition. The process continues until it reaches a leaf node, which indicates the final class or value \cite{decision_trees_Jijo2021Classification}.

%         \begin{figure} [H]
%           \centering
%           \includegraphics[width=0.45\linewidth]{Chapters/Figures/dt-example.png}
...
%         \begin{figure} [H]
%           \centering
%           \includegraphics[width=0.45\linewidth]{Chapters/Figures/SVM.png}
...
% \textbf{miRNA as biomarkers} in oncology. Like coding genes, miRNA
...
% used to reveal miRNA signatures with diagnostic and prognostic value - a
% foundation point for the objectives of this dissertation.

\subsection{\gls{ml} unravelling miRNA as biomarkers}
...
the identification of miRNA as biomarkers in oncological contexts through
\gls{ml} techniques. We know for a fact that miRNA are a class of small
...
revealing latent patterns in miRNA expression data, allowing the
...
pipeline applied to the identification of miRNA as biomarkers for gastric
...
cancer?” Despite growing evidence of the regulatory role of miRNA in
...
process of discovering these miRNA that may be biomarkers, where the
result also has added robustness so that it can be replicated in a real
clinical environment.

\begin{figure}
  \centering
  \includegraphics[width = 1.0\textwidth]{Chapters/Figures/workflow_gastric.png}
...
including miRNA expression profiles and associated clinical information -
...
uniform. Therefore, similar to the study on breast cancer subtyiping with \gls{ml},
...
Tree, Logistic Regression, and K-Nearest Neighbors, all of which were evaluated using metrics such as
...
sensitivity, justifying its selection as the final model. This analysis
...
identify patterns and the relevance of miRNA in this disease. This step
...
\begin{figure}
  \centering
  \includegraphics[width = 0.6\textwidth]{Chapters/Figures/up-down-regulation-azari.png}
  \caption{
    Differential regulation levels of the 29 miRNA selected by the SVM model in the study by \textcite{ml_gastric_Azari2023}. Positive logFC (log fold-change) values indicate overexpressed miRNA (magenta) and negative values indicate underexpressed miRNA (green) in gastric cancer samples compared to healthy tissue.}
  \label{fig:regulation_levels}
\end{figure}

After identifying 29 candidate miRNA from the reduced set of 220
miRNA, the authors proceeded to a validation and refinement stage. This
phase aimed to ensure that the selected miRNA not only stood out
...
miRNA with consistent differential expression in multiple cancer
...
potential of each miRNA, as well as survival analyses to assess their
prognostic value, demonstrating a a better indicator performance when combining
two miRNA together (Figure \ref{fig:roc_azari}).

However, the study's major revelation is not limited to predictive capacity.
The final set of four/five selected miRNA (hsa-miR-21, hsa-miR-133a,
...
confirmed that the genes regulated by these miRNA are involved in
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    \includegraphics[width =\textwidth]{Chapters/Figures/roc_azari.png}
    \caption{}
    \label{fig:roc_azari}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[width =\textwidth]{Chapters/Figures/mirna_to_gene.png}
    \caption{}
  ...
  \caption{(a) ROC curve comparing the diagnostic performance of individual
    ...
    shared regulatory targets across all five miRNA, suggesting
...
It's now a fact that identifying miRNA that are promising biomarkers
...
to ensuring that the identified miRNA were not only statistically
relevant but also biologically plausible and clinically actionable. The final
panel of four miRNA, validated at multiple levels, represents a concrete
...
and adapted to other diseases - including breast cancer, the focus of this
dissertation.

\noindent\rule{\linewidth}{0.4pt}

If the previous study demonstrated how it is possible, through a robust machine
learning pipeline, to reduce thousands of candidate miRNA to a small
...
a clinically identified set of miRNA associated with breast cancer and seeks,
...
the next step: the verification and validation of already known miRNA
...
\begin{figure}
  \centering
  \includegraphics[width = 0.5\textwidth]{Chapters/Figures/pipeline_validation_mirna.png}
...
validating a set of miRNA previously identified as clinically relevant
for breast cancer, using a \gls{ml}-based approach. Unlike exploratory methods, this
...
comprises 1207 breast cancer samples with 1881 miRNA expression profiles
...
studies, the authors manually selected the 36 most promising miRNA for
...
Now, with these 36 miRNA, the authors moved on to the predictive modeling
phase, testing the effectiveness of different classifiers with progressively
smaller subsets of miRNA. Three main classification algorithms were used
...
only three highly informative miRNA could support high-quality
classifications, with performance comparable (or superior) to that obtained
with the complete set of 1881 miRNA. This has significant practical
...
with only three selected miRNA (e.g., IG-3 and CHI2-3 with SVM-RBF). This
...
demonstrate the central role that miRNA can play in the future of cancer
...
\textcite{ml_gastric_Azari2023}, to the rigorous validation of miRNA
...
designed and validated, can effectively exploit the complexity of miRNA
...
% \subsection{Unsupervised Learning in Breast Cancer Subtype Discovery}
% \label{sec:background:unsupervised}
% The studies discussed in the preceding sections establish that machine learning can
% extract meaningful signal from high-dimensional biological data, both for the general
% task of cancer classification and for the more specific problem of identifying miRNA
% expression patterns associated with disease subtypes. However, the approaches examined
% so far share a common assumption: the availability of reliable class labels that supervise
% the learning process. This raises a natural question: \textbf{to what extent does the structure
% implied by those labels reflect genuine, data-intrinsic organisation, rather than the
% boundaries imposed by the labelling scheme itself?} Unsupervised learning offers a
% complementary perspective by letting the data speak without prior annotation, potentially
% revealing subgroup structure that supervised methods, constrained to reproduce known
% categories, may overlook. In the context of mapping miRNA expression profiles to breast
% cancer subtypes, this distinction is particularly relevant: if miRNA signatures carry
% sufficient biological information to drive meaningful clustering, unsupervised methods
% should recover groupings that are at once statistically coherent and clinically
% interpretable. The following subsection reviews the foundations of this approach and
% examines how it has been applied to breast cancer subtype discovery.

% This branch of Machine Learning refers to a family of methods that operate on unlabelled data,
% seeking to discover latent structure without explicit supervision
% \cite{hastie2009elements}. The prototypical task is \textit{clustering}: partitioning a
% dataset into groups such that samples within a group are more similar to each other than
% to samples in other groups. Unlike supervised classification, there is no ground truth to
% optimise against during training; evaluation relies either on internal geometric measures
% or on post-hoc comparison with external labels when available. Unsupervised learning
% has been applied to cancer genomics since the early studies of molecular classification
% \cite{bc_molecular_Perou2000}, where hierarchical clustering on gene
% expression microarray data demonstrated that transcriptomic profiles alone, without
% clinical labels, can recover biologically meaningful tumour classes. More broadly, with
% the increasing availability of clinical and molecular patient data, unsupervised methods
% have gained prominence as tools for \textit{phenomapping}: the delineation of
% therapeutically homogeneous patient subclasses within heterogeneous diseases
% \cite{shah2015phenomapping}.

% A representative example of this paradigm is the work of \citeauthor{ferro2021phenomapping}
% \cite{ferro2021phenomapping}, who applied unsupervised clustering to a cohort of 712
% women with primary breast cancer to identify clinically meaningful patient
% subgroups from biological prognostic parameters. The study is motivated by the high
% heterogeneity of PBC at the clinical, histopathological, and molecular levels, which
% complicates patient management when similar clinical presentations may conceal
% distinct underlying disease profiles. The authors frame this as a patient similarity
% problem and argue that unsupervised learning offers a principled approach to uncovering
% hidden structures in such complex, high-dimensional data.

% The input feature set comprised five continuous quantitative variables: patient age,
% oestrogen receptor (ER) levels, progesterone receptor levels, pS2 protein levels,
% and cathepsin D levels. The restriction to continuous variables was imposed by the
% requirements of the algorithms under consideration; tumour size was excluded due to an
% elevated proportion of missing values (49\%). Rather than committing to a single method,
% the authors compared four clustering algorithms: K-Means,
% self-organising maps (SOM), hierarchical agglomerative clustering
% (HAC), and Gaussian mixture models (GMM); so as to assess whether the identified structure was robust
% across methodological choices.

% Since no external reference partition was available in this historical dataset, model
% selection and evaluation were conducted entirely through internal validation. Evaluating
% clustering solutions is inherently challenging due to the absence of ground-truth labels,
% and the literature distinguishes between \textit{internal} metrics, which assess the
% geometric quality of the clusters without any reference labels, and \textit{external}
% metrics, which compare cluster assignments against a known partition post-hoc. The
% internal metric used throughout this line of work is the \textbf{silhouette score}
% \cite{rousseeuw1987silhouettes}, defined for each sample $i$ as:
...
% \noindent where $a(i)$ is the mean intra-cluster distance from sample $i$ to all other
% samples in its cluster, and $b(i)$ is the mean distance from $i$ to the nearest
% neighbouring cluster. Values close to $+1$ indicate that the sample is well matched to
% its own cluster and poorly matched to neighbouring clusters; values near $0$ indicate
% overlap; and negative values suggest potential misassignment. The overall silhouette
% score is the mean over all samples. In \citeauthor{ferro2021phenomapping}'s study,
% the authors combined the silhouette width with two additional internal indices, 
% over a range of $k = 2$ to $18$ clusters,
% selecting the optimal partition via a rank aggregation procedure. All methods converged
% on $k = 2$; HAC achieved the highest scores across all three measures, whereas the GMM
% performed the poorest. Inter-algorithm agreement was further quantified using Meil{\u a}'s
% Variation of Information \cite{meila2007comparing}, which revealed strong pairwise
% similarity among K-Means, SOM, and HAC, while the GMM diverged from the remaining
% methods.

% When ground-truth labels are available for post-hoc validation, a richer set of
% external metrics can be brought to bear. \textbf{Cluster purity} measures, for each
% cluster, the proportion of samples belonging to the most frequent true class within that
% cluster, aggregated as a weighted average across all clusters:
...
% \noindent where $L_j$ is the set of samples with true label $j$ and $N$ is the total
% number of samples. Purity ranges from 0 to 1, with higher values indicating better
% alignment between clusters and true classes. However, purity alone does not penalise
% trivial solutions in which every sample is placed in its own cluster; it is therefore
% complemented by agreement-based measures. The \textbf{Adjusted Rand Index} (ARI)
% \cite{hubert1985comparing} counts the fraction of sample pairs for which the clustering
% and the ground-truth labelling agree, correcting for the agreement expected by chance:
...
% \noindent ARI $= 1$ indicates perfect agreement; ARI $= 0$ indicates agreement no better
% than random; negative values indicate systematic disagreement. Similarly, the
% \textbf{Adjusted Mutual Information} (AMI) \cite{vinh2010information} quantifies the
% information shared between the cluster assignments and the true labels, normalised and
% adjusted for chance:

% \begin{equation}
%     \text{AMI} = \frac{I(U;V) - \mathbb{E}[I(U;V)]}{\text{avg}(H(U), H(V)) -
%     \mathbb{E}[I(U;V)]}
%     \label{eq:ami}
% \end{equation}

% \noindent where $I(U;V)$ is the mutual information between the clustering $U$ and the
% true labelling $V$, and $H(\cdot)$ denotes entropy. Like ARI, AMI $= 1$ is perfect and
% AMI $= 0$ is random. Together, purity, ARI, and AMI provide a complementary view of
% how well an unsupervised partition recovers a known reference labelling.

% The two clusters identified by HAC exhibited a clear clinical differentiation. The
% first cluster comprised younger patients with lower levels of ER, PgR, and cathepsin D;
% the second cluster consisted of older, predominantly postmenopausal women with elevated
% hormone receptor levels and higher cathepsin D values. Variable importance analysis,
% carried out separately for each algorithm using method-specific procedures, consistently
% ranked age, ER, and PgR as the three most influential features in defining the partition.
% These findings are consistent with established biological evidence linking ER and PgR
% positivity with age and menopausal status in breast tumours, reinforcing the validity of
% the identified subgroups beyond the statistical criteria used for selection.

% This study highlights several methodological considerations relevant to the
% application of unsupervised learning in a clinical oncology setting. First, the choice of
% clustering algorithm substantially affects both the quality and the interpretability of the
% resulting partition, underscoring the importance of systematic comparison rather than
% reliance on a single method. Second, the cluster validity problem - the principled
% selection of both the algorithm and the number of clusters - requires explicit attention
% through internal validation indices, which are often overlooked in practice. Third,
% post-hoc variable importance assessment provides an interpretable link between the
% statistical partition and the underlying biology, bridging the gap between data-driven
% discovery and domain knowledge.

\subsection{Unsupervised Learning in Breast Cancer Subtype Discovery}
\label{sec:background:unsupervised}

The studies discussed in the preceding sections establish that machine learning can extract meaningful signal from high-dimensional biological data, both for the general task of cancer classification and for the more specific problem of identifying miRNA expression patterns associated with disease subtypes. However, the approaches examined so far share a common assumption: the availability of reliable class labels that supervise the learning process. This raises a natural question: \textbf{to what extent does the structure implied by those labels reflect genuine, data-intrinsic organisation, rather than the boundaries imposed by the labelling scheme itself?} Unsupervised learning offers a complementary perspective by allowing the data to be analysed without prior annotation, potentially revealing subgroup structure that supervised methods, constrained to reproduce known categories, may overlook. In the context of mapping miRNA expression profiles to breast cancer subtypes, this distinction is particularly relevant. If miRNA signatures carry sufficient biological information, unsupervised methods should recover groupings that are both statistically coherent and clinically interpretable.

This branch of Machine Learning refers to a family of methods that operate on unlabelled data, seeking to discover latent structure without explicit supervision \cite{hastie2009elements}. Rather than learning a mapping between input variables and predefined outcomes, these approaches aim to characterise the intrinsic organisation of the data itself. As noted by \citeauthor{https://doi.org/10.1007/s00167-022-07233-7} \cite{https://doi.org/10.1007/s00167-022-07233-7}, unsupervised methods are particularly useful for analysing and simplifying high-dimensional datasets, identifying hidden patterns that may not be immediately apparent through conventional analyses. Among the most widely used techniques are clustering algorithms such as K-Means and hierarchical clustering, which group observations according to similarity, and dimensionality reduction methods such as Principal Component Analysis (PCA) and factor analysis, which provide complementary views of the structure underlying complex datasets.

The application of these methods to cancer genomics is particularly compelling. High-throughput technologies routinely generate measurements involving hundreds or thousands of molecular features, often exhibiting complex relationships that are not immediately captured by clinical annotations. Unsupervised learning therefore provides a framework for investigating whether molecular data contain intrinsic organisation that reflects underlying biological processes. Early molecular classification studies demonstrated that transcriptomic profiles alone could recover biologically meaningful tumour classes without explicit supervision \cite{bc_molecular_Perou2000}, suggesting that at least part of the heterogeneity observed in cancer emerges naturally from the structure of the molecular data. In the present work, this motivates the investigation of whether miRNA expression profiles encode a geometry that aligns with established breast cancer subtype definitions.

Evaluating clustering solutions presents unique challenges because no ground-truth labels are available during training. Consequently, clustering performance is often assessed using internal validation measures that quantify the geometric quality of the resulting partition. One of the most widely used metrics is the \textbf{silhouette score} \cite{rousseeuw1987silhouettes}, defined for each sample $i$ as
...
\noindent where $a(i)$ denotes the mean intra-cluster distance between sample $i$ and all other samples assigned to the same cluster, and $b(i)$ corresponds to the mean distance between $i$ and the nearest neighbouring cluster. Values approaching $+1$ indicate that a sample is well matched to its assigned cluster and poorly matched to neighbouring clusters, whereas values close to $0$ suggest overlap between groups.

When reference labels are available, external measures can be used to assess the extent to which the identified clusters align with known categories. In the context of breast cancer, this enables the comparison of unsupervised partitions against established subtype annotations. \textbf{Cluster purity} measures the proportion of observations within each cluster belonging to the most frequent reference class:
...
\noindent where $L_j$ denotes the set of samples with true label $j$, $C_k$ represents the set of observations assigned to cluster $k$, and $N$ is the total number of samples.

Agreement-based measures further account for chance correspondence between clustering assignments and reference labels. The \textbf{Adjusted Rand Index} (ARI) \cite{hubert1985comparing} is defined as
...
\noindent where values of $1$ indicate perfect agreement and values close to $0$ correspond to agreement expected by chance. Similarly, the \textbf{Adjusted Mutual Information} (AMI) \cite{vinh2010information} quantifies the information shared between clustering assignments and reference labels while correcting for chance:

\begin{equation}
\text{AMI} = \frac{I(U;V) - \mathbb{E}[I(U;V)]}{\mathrm{avg}(H(U), H(V)) - \mathbb{E}[I(U;V)]}
\label{eq:ami}
\end{equation}

\noindent where $I(U;V)$ represents the mutual information between the clustering solution $U$ and the reference partition $V$, and $H(\cdot)$ denotes entropy.

Together, these metrics provide complementary perspectives on cluster validity. Internal measures evaluate whether a clustering solution captures coherent geometric organisation within the data itself, whereas external measures assess the extent to which this organisation aligns with existing biological knowledge. This distinction is central to the present work. Rather than assuming that breast cancer subtype labels necessarily reflect the organisation of miRNA expression space, unsupervised learning provides a means of investigating whether these clinically defined categories emerge naturally from the underlying molecular geometry.
...
key elements in the classification of tumor biopsies, directly outlining some
possible paths for this thesis.

One of the first aspects to highlight is the \textbf{importance of differential
  expression analysis as an initial step in feature selection}. This approach is 
very common and proved effective in reducing dimensionality in scenarios with high feature
density, such as miRNA expression data \cite{Azari2023}. This strategy allows the analysis to
...
with only a fraction of the input data (such as subsets of 3 to 10 miRNA)
...
finding a small group of miRNA that would have the same performance as
...
Moving beyond the supervised paradigm, the studies reviewed in this chapter also motivate
the adoption of an \textbf{unsupervised learning approach} as a complementary axis of
investigation. While the supervised methods discussed above are effective at building
classifiers when reliable labels are available, they do not allow us to question whether
the data itself organises around those labels in a meaningful way. As demonstrated by
\textcite{ferro2021phenomapping}, unsupervised clustering can reveal intrinsic patient
subgroup structure that is both statistically robust and biologically interpretable,
without ever consulting class annotations during training. Transposing this rationale
to the context of this thesis, a natural question arises: if miRNA expression
profiles carry sufficient discriminative signal to support supervised classification,
do they also encode an intrinsic structure that clustering algorithms can recover
independently? Exploring this question from the right perspective, it provides an
independent validation of the biological relevance of the miRNA features selected
during the supervised pipeline, and if the same features drive both a good classifier
and a meaningful clustering, the evidence for their discriminative power is substantially
strengthened. This unsupervised component will therefore
be designed not as an alternative to the supervised approach, but as a structural
complement to it, deepening the understanding of how miRNA expression relates to the
heterogeneity of breast cancer subtypes.
