...
same clinical subtype may respond differently to the same treatments \cite{need_for_subtype_treatments_Testa2020Breast, bc_subtypes_Haque2012}.
...
molecular targets \cite{complete_analysis_bca_Łukasiewicz2021Breast}, substantial limitations remain, particularly related to
...
and leaves chemotherapy as the main treatment option \cite{tnbc_therapies_Mahalingam2020The}. 
...
% \noindent Because the objective in Equation~\ref{eq:kmeans_objective} is non-convex, the
% solution depends on the choice of initial centroids. To mitigate convergence to poor local
% minima, the K-Means\texttt{++} initialisation strategy \cite{arthur2007kmeans} selects initial
% centroids with probability proportional to their squared distance from already-chosen centroids,
% yielding both faster convergence and consistently better solutions in practice.

% Unsupervised learning methods have been applied to genomic and transcriptomic cancer data since
% the early studies of molecular classification \cite{perou2000molecular, eisen1998cluster}.
% Notably, hierarchical clustering on gene expression microarray data was used to identify
% intrinsic breast cancer subtypes in the landmark work of \citeauthor{perou2000molecular}
% \cite{perou2000molecular}, demonstrating that transcriptomic profiles alone, without clinical
% labels, can recover biologically meaningful tumour classes.

% % ----------------------------------------------------------
% \subsection{Evaluation Metrics}
% \label{sec:unsupervised:metrics}
% \paragraph{Evaluation Metrics for Clustering} Evaluating clustering solutions is inherently 
% challenging due to the absence of ground-truth labels. In the context of this work, where the goal 
% is to recover known breast cancer subtypes from transcriptomic data, we have access to true 
% class labels that can be used for external validation. However, since the clustering 
% algorithm does not use these labels during model fitting, we also consider internal metrics 
% that assess the geometric quality of the clusters without reference to the true classes.
...
\cite{perou2000molecular}, where hierarchical clustering on gene
