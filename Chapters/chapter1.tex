...
First, we intend to assess on a pre-selected set of patients (selected by NOVA 
Medical School team) the performance supervised models for the classification 
...
discriminative and can be used to classify wether a patient belongs to a given 
...
Even though the base study we've done seems promising, there are several challenges to overcome
in order to achieve the desired results such as: \\ \textbf{1. Inter-Tumor
...
Having a control would be an important step toward increasing the
generalizability of our model. Using a negative sampling, it would not only
allow us to expand the problem to include the distinction between healthy and
sick individuals, but also improve the identification of discriminative
...
framework integrates and compares different \gls{ml} approaches and how wee can 
leverage the use of group-based approaches for feature extraction and feature 
selection of \gls{bc} subtypes.
...
In addition to the classification process, this framework includes a statistical 
feature extraction through the use of ANOVA F-test, in order to confer robustness 
to the decisions generated by the model. This
...
into medical practice. The findings of this work could be tested using both
